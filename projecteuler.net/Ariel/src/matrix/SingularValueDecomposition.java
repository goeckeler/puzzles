	...
	public SingularValueDecomposition(Matrix Arg) {
		...
		for (int k = 0; k < Math.max(nct, nrt); k++) {
			if (k < nct) {
				...
				s[k] = 0;
				for (int i = k; i < m; i++) {
					s[k] = Hypotenuse.hypot(s[k], A[i][k]);
				}
				if (s[k] != 0.0) {
					if (A[k][k] < 0.0) {
						s[k] = -s[k];
		...
		while (p > 0) {
			...
			switch (kase) {
				...
				case 1 : {
					...
					e[p - 2] = 0.0;
					for (int j = p - 2; j >= k; j--) {
						double t = Hypotenuse.hypot(s[j], f);
						double cs = s[j] / t;
						double sn = f / t;
						s[j] = t;
						if (j != k) {
				...
				case 3 : {
					...
					double scale = Math.max(Math.max(Math.max(Math.max(Math
							.abs(s[p - 1]), Math.abs(s[p - 2])), Math
